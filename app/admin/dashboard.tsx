...
type Row = { id: string | number; admin_version: number; customer_name?: string; customer_email?: string; customer_phone?: string; start_date?: string; end_date?: string; total_price?: number; total_days?: number; status?: string; payment_status?: string; payment_method?: string; booking_source?: string; total_scooters?: number; rating?: number; review_text?: string };
...
export default function Dashboard() {
  ...
  async function add(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (await mutate('reservations', { ...data, total_price: Number(data.total_price), admin_request_id: requestId.current }, 'POST')) { form.reset(); requestId.current = ''; setManual(false); }
  }
  return <>
    ...
    {manual && tab === 'reservations' && <form className="ec-card ec-form" onSubmit={add}><h3>Ajouter une réservation WhatsApp</h3><p>Un scooter par réservation. Les deux dates sont incluses. Le prix total convenu est saisi en MAD ; aucun paiement n’est déclenché.</p><fieldset disabled={busy} className="ec-grid">
      ...
      <label>Début<input name="start_date" type="date" required /></label><label>Fin incluse<input name="end_date" type="date" required /></label><label>Prix total (MAD)<input name="total_price" type="number" min="0" max="99999999.99" step="0.01" required /></label><label>Mode de paiement<select name="payment_method"><option value="cash">Espèces</option><option value="online">En ligne</option></select></label>
    ...
    {loading ? <p role="status">Chargement…</p> : <div className="ec-list">{!rows.length && !error && <div className="ec-card">Aucun résultat.</div>}{rows.map(row => <article className="ec-card" key={String(row.id)}>
      {tab === 'settings' ? <form className="ec-form" onSubmit={async e => { e.preventDefault(); const total = Number(new FormData(e.currentTarget).get('total')); if (window.confirm(`Définir le parc à ${total} scooter(s) ?`)) await mutate('settings', { id: String(row.id), admin_version: row.admin_version, total_scooters: total }); }}><h3>Nombre de scooters</h3><p>Une réduction est refusée si elle rend une réservation active impossible. Zéro ferme les nouvelles réservations.</p><label>Parc disponible<input key={`${row.id}-${row.admin_version}`} name="total" type="number" min="0" max="10000" defaultValue={row.total_scooters} required /></label><button disabled={busy}>Enregistrer le parc</button></form> : <>
        ...
        {tab === 'reservations' ? <><div className="ec-details"><p><strong>{row.start_date} → {row.end_date}</strong><br />{row.total_days} jour(s) · 1 scooter</p><p><strong>{Number(row.total_price).toFixed(2)} MAD</strong><br />{labels[row.payment_status ?? '']} · {row.payment_method === 'cash' ? 'Espèces' : 'En ligne'}</p></div><div className="ec-actions">
