...
type Result = { scooterQuantity: number; state: string; reservationId: string; eur: string; mad: string; mode: string };
function PaymentResult() {
  ...
  return <main className="min-h-screen bg-[#F3EFE7] px-5 py-28 text-[#49372D]"><div className="mx-auto max-w-xl rounded-[30px] bg-[#FFFDF8] p-8">
    ...
    {paid && <p className="mt-5 leading-7">Reservation #{result.reservationId} · {result.scooterQuantity} scooter(s) · Paid {result.eur} EUR ({result.mad} MAD).{result.mode === 'sandbox' && ' Sandbox test — no real money was charged.'}</p>}
